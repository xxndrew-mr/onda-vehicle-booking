...
- Frontend: `AuthContext` (identitas via `/api/auth/me`), halaman di `src/pages/`. Navbar menampilkan menu Approval hanya bila `is_supervisor || GA || ADMIN`; `is_supervisor` = leader departemen Lark ATAU ada bawahan (`hasSubordinates`, dicek di `sso.js` saat login). Divisi pemohon disimpan di `bookings.requester_department` (snapshot saat buat) dan ditampilkan di approval/detail/security. Halaman booking punya seksi "Riwayat Booking Saya" (filter client-side `requester_id == user.id` dari data GET /api/bookings yang sama dengan kalender) dengan badge status (`STATUS_META`), info audit approval, dan aksi Detail/Batalkan.
