...
- Avatar: URL foto Lark (`users.avatar_url`, dari Contact API `avatar_240`, di-refresh tiap login/reset) adalah **CDN publik** (feishucdn/larksuitecdn) — dipakai LANGSUNG di `<img>` via komponen `src/components/Avatar.js` (`referrerPolicy="no-referrer"` cegah 403 hotlink; `onError` → fallback inisial berwarna). Avatar current user ada di JWT (`avatar`) + `/api/auth/me`. Avatar pemohon di kartu/tabel booking via `LEFT JOIN users u ON b.requester_id = u.lark_user_id` (`requester_avatar`) di `bookings/index.js` GET, `pending.js`, `history.js`.
...
- Armada: `src/lib/vehicleStatus.js` (konstanta status, `isVehicleAvailable` = 'Ready'), API `vehicles/index.js` (GET semua + POST GA/ADMIN) & `vehicles/[id].js` (PATCH GA/ADMIN), halaman `/armada`. Status kendaraan (`Ready|In Use|Maintenance|Unavailable`) langsung memengaruhi ketersediaan booking — hanya 'Ready' yang bisa dipesan (dropdown booking difilter + POST divalidasi).
