...
- `src/lib/notify.js` — notifikasi bot Lark berupa KARTU interaktif via `sendLarkCard()` (im/v1/messages `msg_type: interactive`, tenant token; `sendLarkMessage()` teks masih tersedia di lark.js). Kartu: judul berwarna (blue/green/red) + detail lark_md + TOMBOL link (bukan URL panjang) yang dibungkus `larkAppLink()` → terbuka di dalam klien Lark agar SSO otomatis. `notifyBookingCreated` (→ supervisor / GA) & `notifyTransition` (→ GA berikutnya / pemohon saat approve/reject). Best-effort: dibungkus try/catch di API, kegagalan TIDAK menggagalkan booking/approval. Butuh setup Lark: kapabilitas Bot + scope `im:message:send_as_bot` + visible range + rilis versi. Tanpa itu, notifikasi diam-diam gagal (log saja).
...
String status dipakai oleh API, halaman approval, dan timeline booking. Halaman booking (`index.js`) memakai **timeline harian custom** `src/components/VehicleTimeline.js` ala booking room Lark (kendaraan = baris kiri sticky, jam = sumbu horizontal; BUKAN FullCalendar — resource-timeline FullCalendar berbayar, dependensinya sudah dihapus). Timeline HANYA menampilkan booking AKTIF (`Approved` hijau, `Pending*` kuning); `Rejected*` & `Cancelled*` DISEMBUNYIKAN (`isActiveBooking`) — tetap muncul di Riwayat user. Booking dibuat dengan DRAG horizontal di baris kendaraan Ready (snap 30 menit; tap tanpa drag = slot 1 jam untuk layar sentuh) → modal dengan kendaraan itu terpilih + jam bisa diedit (`datetime-local`); pilihan kendaraan di modal = Ready yang BEBAS di rentang itu (`vehiclesFreeForSlot`, cek bentrok client-side; server tetap validasi 409). Baris non-Ready tampil redup dan tidak bisa di-drag.

- Booking baru: pemohon punya supervisor → `Pending Supervisor` (dengan `supervisor_id`); tidak punya → `Pending GA`. Supervisor di-resolve di `lark.js`: `contact.leader_user_id` (direct manager), FALLBACK ke leader departemen dengan MENELUSURI hierarki ke atas (`findSupervisorUpChain`: sub-divisi tanpa leader / leader = dirinya → naik ke `parent_department_id` sampai ketemu leader valid; mentok → `Pending GA`). Semua open_id.
