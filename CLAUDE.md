...
- Logout: tombol "Keluar" di Navbar → modal konfirmasi INSTAN → POST `/api/auth/logout` (hapus cookie) → `window.location.assign('/keluar')`. Pakai `assign` (bukan `href='/'` yang tak reload bila URL sama, dan di dalam Lark auto-login lagi). `/keluar` publik: dikecualikan di `proxy.js` matcher + `AuthContext` PUBLIC_PATHS, dan `_app.js` menyembunyikan navbar/footer untuk `/keluar` & `/auth-error` (BARE_PAGES).
